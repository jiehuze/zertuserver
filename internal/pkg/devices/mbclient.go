...
const (
	FuncCodeWriteSingleEXRegister = 7
)
...
func NewClient2(packager modbus.Packager, transporter modbus.Transporter) *client {
...
func (mb *client) ReadHoldingRegisters(address, quantity uint16) (results []byte, err error) {
	if quantity < 1 || quantity > 5120 {
		err = fmt.Errorf("modbus: quantity '%v' must be between '%v' and '%v',", quantity, 1, 5120)
		return
	}
	request := modbus.ProtocolDataUnit{
...
//	Register address      : 2 bytes
//	Register value        : 2 bytes
func (mb *client) WriteSingleExRegister(address, value uint16) (results []byte, err error) {
	request := modbus.ProtocolDataUnit{
		FunctionCode: FuncCodeWriteSingleEXRegister,
		Data:         dataBlock(address, value),
	}
	response, err := mb.send(&request)
	if err != nil {
...
func (mb *client) send(request *modbus.ProtocolDataUnit) (response *modbus.ProtocolDataUnit, err error) {
